...
WITH  base AS (SELECT to_char(sysdate-1,'DD.MM.YYYY') FECHA,
      ...
      mob_asp_day AS    (SELECT 'MOBILEUM_ASP_DAY' TABLA,
                        ...
                        GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      mob_asp_bh  AS    (SELECT /*+ materialize*/
                                to_char(FECHA,'DD.MM.YYYY') FECHA,
                                COUNT(1) CNT_FILAS
                        FROM  MOBILEUM_ASP_BH
                        GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      mob_asp_ibhw  AS  (SELECT /*+ materialize*/
                                to_char(FECHA,'DD.MM.YYYY') FECHA,
                                COUNT(1) CNT_FILAS
                        FROM  MOBILEUM_ASP_IBHW
                        GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cdc_hour   AS (SELECT /*+ materialize*/
                                to_char(FECHA,'DD.MM.YYYY') FECHA,
                                COUNT(1) CNT_FILAS
                        FROM  TEC_CE_CDC_TPS_HOUR
                        GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cdc_day    AS (SELECT /*+ materialize*/
                                to_char(FECHA,'DD.MM.YYYY') FECHA,
                                COUNT(1) CNT_FILAS
                        FROM  TEC_CE_CDC_TPS_DAY
                        GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cdc_bh  AS  (SELECT /*+ materialize*/
                              to_char(FECHA,'DD.MM.YYYY') FECHA,
                              COUNT(1) CNT_FILAS
                      FROM  TEC_CE_CDC_TPS_BH
                      GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cdc_ibhw  AS  (SELECT /*+ materialize*/
                                to_char(FECHA,'DD.MM.YYYY') FECHA,
                                COUNT(1) CNT_FILAS
                        FROM  TEC_CE_CDC_TPS_IBHW
                        GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cm_hour AS  (SELECT /*+ materialize*/
                              to_char(FECHA,'DD.MM.YYYY') FECHA,
                              COUNT(1) CNT_FILAS
                      FROM  TEC_CE_CM_TPS_HOUR
                      GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cm_day  AS  (SELECT /*+ materialize*/
                              to_char(FECHA,'DD.MM.YYYY') FECHA,
                              COUNT(1) CNT_FILAS
                      FROM  TEC_CE_CM_TPS_DAY
                      GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cm_bh AS  (SELECT /*+ materialize*/
                            to_char(FECHA,'DD.MM.YYYY') FECHA,
                            COUNT(1) CNT_FILAS
                    FROM  TEC_CE_CM_TPS_BH
                    GROUP BY to_char(FECHA,'DD.MM.YYYY')),
      tec_cm_ibhw  AS  (SELECT  /*+ materialize*/
                                to_char(FECHA,'DD.MM.YYYY') FECHA,
                                COUNT(1) CNT_FILAS
